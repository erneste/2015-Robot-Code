...
int rotclock = 0;
int mordorleft = 0; //used to determine if the bumper is down
task main()
{
	waitForStart();
	while(true)
	{
// This is the degree storage
	if (SensorValue[gyro] < -1)
			rotclock= rotclock+ SensorValue[gyro]
	;
		if (joy1Btn(leftBumper)==true)
			mordorleft= 1
		;
		if (joy1Btn(leftBumper)==false)
			mordorleft= 0
		;
	...
	if (mordorleft==1) //activateflybywire
			motor[frontLeft] = y1 + x1 + x2 +rotclock;
